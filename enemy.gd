...
func _update_floating_movement(_delta: float) -> void:
	...
	if not is_chasing:
		...
		# Add controlled floating motion while chasing (prevent flying to infinity)
		var float_adjustment = sin(time * float_speed * 1.5) * 15.0  # Reduced amplitude
		# Only apply if it won't cause extreme upward velocity
		if velocity.y + float_adjustment < 200.0 and velocity.y + float_adjustment > -200.0:
			velocity.y += float_adjustment
...
func _move_towards_player(delta: float) -> void:
	...
	# Move towards player but with velocity limits
	...
	# Clamp velocities to prevent flying to infinity
...
func die() -> void:
	...
	# Drop healing pickup when enemy dies
...
func _drop_healing_pickup() -> void:
	...
	# Apply launch velocity for physics drop
	var launch_x = randf_range(-50.0, 50.0)
	var launch_y = randf_range(-100.0, -50.0)
	healing_pickup.linear_velocity = Vector2(launch_x, launch_y)
	
	print("Enemy dropped healing pickup with gravity!")

# Pickup detection handled by player's active scanning system
